...
| Moonveil Premium Monthly | Auto-renewing subscription | `moonveil_premium_monthly`; RevenueCat entitlement `premium` | Premium subscription, local profile, full numerology/profile access, custom spreads, backup/export, sync and premium content/modules according to the final offering. | Seeded in backend production data. Final Play/App Store/RevenueCat offering and price still require configuration and testing. |
...
| Backup & Export | Entitlement | RevenueCat identifier `backup` | Optional versioned cloud backup of local user payload. | Backend implementation exists on open backend PR #75; not merged at review time. |
...
| Area | Verified state on 23 July 2026 |
|---|---|
...
| Account deletion | `DELETE /me` plus RevenueCat customer deletion and removal of linked billing records exists in open backend PR #75. Do not declare production availability until merged, deployed and exposed in mobile UI. |
| Cloud backup | Versioned `GET/PUT/DELETE /me/backup`, arbitrary JSON payload and 5 MB limit exist in open backend PR #75. Payload is not end-to-end encrypted by the current code. |
| Public backend privacy page | A short `/privacy` page exists only in open backend PR #75. It is not sufficient as the complete public policy and should link to the canonical policy in this repository. |
| Mobile billing SDK | The reviewed mobile `package.json` did not contain a RevenueCat/Purchases dependency. Purchase and restore flows must be verified in the actual release branch/binary before launch. |
...
1. show a separate, granular opt-in explaining exactly what leaves the device;
2. collect explicit consent where Article 9 applies;
3. allow backup deletion and consent withdrawal even after Premium expires;
...
- `[[HOSTING_PROVIDER]]` — VPS/backend/PostgreSQL/logs/backups; execute a DPA and identify data location.
...
- Backend PR #75 and mobile deletion UI are not confirmed production-ready.
- Final RevenueCat SDK/offering and mobile product configuration are not verified.
- Cloud-backup consent, sensitive-data UX and production security controls are not verified.
- Backup/log/support retention values are not final.
