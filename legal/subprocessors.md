...
| Contabo GmbH / applicable contracting entity | EU VPS/container hosting for the Moonveil API, PostgreSQL and operational logs | Processor | Account identifiers, optional email, auth links, billing metadata, optional backup payload, logs and security metadata | Production server is in Contabo region EU; apply contractual transfer safeguards to any ancillary processing outside the EEA where required | The production-VPS DPA became active on 10 August 2026 and must be archived privately. Provider Auto Backup remains disabled. The provider firewall allows only TCP 22/80/443 and UDP 443 inbound before a default drop rule. Restricted local database backups rotate by age and count. | Archive the DPA PDF privately, retain the port plan and firewall evidence, and complete the non-destructive restore test. SSH remains open to any source because the current GitHub-hosted deploy runner has no fixed address; replace this with a fixed runner/VPN before narrowing it. |
| Cloudflare, Inc. / applicable Cloudflare entity | R2 object storage, CDN/DNS and security | Processor for configured storage/delivery; may process request metadata under its service terms | Public content assets and IP/request metadata; encrypted/authenticated restic objects may encapsulate production PostgreSQL data, including account, billing and optional user-backup records | Global provider network; Cloudflare data-processing terms and applicable SCCs | The dedicated `moonveil-backups-prod` bucket is private, has no public development URL or custom domain, and uses an object read/write token scoped only to that bucket. Daily restic snapshots are encrypted before upload and retained off-site for 14 days; the first production snapshot was created on 10 August 2026. | Retain the applicable DPA/transfer configuration, rotate the bucket-scoped token when required, monitor daily snapshots and retain a successful restore/integrity test. |
